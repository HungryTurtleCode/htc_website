...
We have already covered how to build a basic factory when we built the quizMetrics factory in a [previous part]({{site.baseurl}}/projects/angular-quiz-app/8-dependency-injection/){: target="_blank"}<!--_-->. In this part, we will build another factory; this time to mock data coming from an API. Let&#8217;s continue using Angular factories to further separate concerns.
...
[The next part can be found here]({{site.baseurl}}/projects/angular-quiz-app/10-ng-class/)
...
In [part 10]({{site.baseurl}}/projects/angular-quiz-app/10-ng-class/) we will use the quiz JSON we now have access to and start creating the bootstrap markup for the quiz.
